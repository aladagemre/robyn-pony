...
@pytest.fixture
def test_user_data():
    return {"name": "Emre", "email": "emre@example.com", "password": "123456"}
...
def test_user_login(server, test_user_data, create_test_user):
    ...
    response = requests.post(
        f"{server}/login", json=login_data, headers={"Content-Type": "application/json"}
    )
...
def test_me_endpoint(server, test_user_data, create_test_user):
    ...
    response = requests.get(
        f"{server}/me", headers={"Authorization": f"Bearer {token}"}
    )
...
def test_login_invalid_credentials(server, test_user_data, create_test_user):
    login_data = {"email": test_user_data["email"], "password": "wrong_password"}
    response = requests.post(
        f"{server}/login", json=login_data, headers={"Content-Type": "application/json"}
    )
...
def test_me_endpoint_invalid_token(server):
    response = requests.get(
        f"{server}/me", headers={"Authorization": "Bearer invalid_token"}
    )
